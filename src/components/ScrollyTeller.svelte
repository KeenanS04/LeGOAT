<script>
  ...
  },
  {
    ...
    content: "LeBron wins the NBA MVP award in 2012 and 2013, solidifying his status as the league's top player during the Heat's championship era."
  },
  {
    title: "Championship Success: The Heat Era",
    content: "LeBron leads Miami to NBA championships in 2012 and 2013, overcoming the Oklahoma City Thunder and the San Antonio Spurs. These victories mark the fulfillment of his championship aspirations and contribute to his growing legacy."
  },
  {
  ...
  },
  {
    ...
    content: "Securing another gold medal with Team USA in 2012, LeBron further cemented his legacy as a basketball icon. His contributions were key in maintaining Team USA's dominance in international basketball."
  },
  {
  ...
  },
  {
    ...
    content: "Throughout his second stint with Cleveland, LeBron's Cavaliers consistently overcome Eastern Conference rivals, including memorable performances against the Toronto Raptors, earning the nickname 'LeBronto' for his domination of the team in the playoffs."
  },
  {
    title: "Scoring Jounrey: (input year)",
    content: ""
  ...
  $: mapVisible = [1, 3, 9, 17, 22].includes(index);
  $: chartVisible = [7, 15, 20, 26].includes(index);
      ...
      <section>
        {#if i === index}
          <div in:fade={{duration: 500 }} out:fade={{duration: 500 }}>
            <h1>{section.title}</h1>
            <p id='left'>{@html section.content}</p>
          </div>
        {/if}
      </section>
    {/each}
